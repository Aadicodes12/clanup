...
const HowItWorksSteps = () => {
  ...
  const steps = [
    {
      icon: <Sparkles className="w-6 h-6 md:w-8 md:h-8 text-[#7be382]" />,
    ...
    {
      icon: <Users className="w-6 h-6 md:w-8 md:h-8 text-[#7be382]" />,
    ...
    {
      icon: <Code className="w-6 h-6 md:w-8 md:h-8 text-[#7be382]" />,
    ...
    <section ref={sectionRef} className="w-full py-16 md:py-24 bg-black text-white relative z-20 overflow-hidden">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="absolute left-[15px] top-0 bottom-0 w-[2px] bg-[#7be382]/30 md:hidden"></div>
          <div
            className={cn(
              "absolute left-[15px] top-0 bottom-0 w-[2px] bg-[#7be382] md:hidden transition-all duration-1000 ease-out",
              isVisible ? "h-full" : "h-0"
            )}
          ></div>
          ...
          <div className="flex flex-col gap-y-10 md:gap-y-0">
            {steps.map((step, index) => (
              ...
              >
                {/* Content Side */}
                <div className={cn(
                  "w-full md:w-1/2 flex pl-10 md:px-12",
                  index % 2 === 0 ? "justify-start md:justify-end" : "justify-start md:justify-start"
                )}>
                  <Card
                    className={cn(
                      "w-full max-w-md bg-neutral-900 border-neutral-800 p-4 md:p-6 transition-all duration-500 hover:scale-105 hover:border-[#7be382] text-left",
                      isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
                    )}
                    style={{ transitionDelay: `${index * 200}ms` }}
                  >
                    <div className="mb-3 md:mb-4 flex justify-start">{step.icon}</div>
                    <CardTitle className="text-lg md:text-xl font-bold font-sora mb-2 md:mb-3 text-white">
                      {step.title}
                    </CardTitle>
                    <CardContent className="p-0 text-gray-400 font-sora text-xs md:text-sm leading-relaxed">
                      {step.description}
                    </CardContent>
                  </Card>
                </div>
                ...
                {/* Center Circle */}
                <div className="absolute left-[15px] md:left-1/2 top-0 md:top-1/2 md:-translate-y-1/2 -translate-x-1/2 z-10">
                  <div className={cn(
                    "w-8 h-8 md:w-12 md:h-12 rounded-full bg-[#7be382] flex items-center justify-center text-black font-bold text-sm md:text-xl shadow-[0_0_20px_rgba(123,227,130,0.3)] transition-all duration-500",
