    ...
    <section ref={sectionRef} className="w-full py-16 md:py-24 bg-black text-white relative z-20">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="relative max-w-sm mx-auto py-8">
          ...
          <div className="absolute left-[19px] md:left-[23px] top-0 bottom-0 w-[2px] bg-[#7be382]/30"></div>
          {/* Animated Vertical Line (foreground) - visible on all screen sizes */}
          <div
            className={cn(
              "absolute left-[19px] md:left-[23px] top-0 bottom-0 w-[2px] bg-[#7be382]",
          ...
          <div className="flex flex-col gap-y-12">
            {steps.map((step, index) => (
              <div key={index} className="relative flex items-start gap-4 md:gap-8 w-full">
                ...
                <div className="relative z-10 w-8 h-8 md:w-10 md:h-10 rounded-full bg-[#7be382] flex items-center justify-center text-black font-bold text-base md:text-lg flex-shrink-0">
                  {index + 1}
                </div>
                <Card
                  className="flex-grow bg-neutral-900 border-neutral-800 p-5 transition-transform duration-300 hover:scale-105 hover:border-[#7be382] text-left max-w-[calc(100% - 40px - 1rem)] md:max-w-none"
                >
